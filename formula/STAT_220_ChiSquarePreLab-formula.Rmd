...
$$
H_0: p_{\text{not too happy}} = 0.14, p_{\text{pretty happy}} = 0.56, p_{\text{very happy}} = 0.3 \\
...
We can assume all the expected counts are greater than 5, because all the observed counts are so much greater. 
...
```{r}
chisq.test(tally(~general_happiness, data = GSS, useNA = "no"), p = c(0.14, 0.56, 0.3))
```

Our test statistic is 0.218, we have 2 degrees of freedom, and our p-value is 0.8965. 

What is the conclusion to the test?

We fail to reject, because the p-value is $>0.05$. We do not have evidence to suggest the proportions are different than those specified in the null. 
...
Again, all our observed counts are significantly greater than 5, so we can assume the expected counts would be as well. 
...
Our test statistic is 201.65, our degrees of freedom are 8, and the p-value is approximately 0. 

What is the conclusion of the test?

Reject the null hypothesis. We have evidence to suggest there is a relationship between marital status and general happiness. 
