...
```{r}
gf_histogram(~number_of_hours_worked_last_week, data = GSS, binwidth = 15)
```
...
In order to get a number out, we need to tell R to ignore those missing values when we compute the mean. There are a couple ways to do this. One is to add the  `na.rm` argument to the summary statistic function. 

```{r}
mean(~number_of_hours_worked_last_week, data = GSS, na.rm = TRUE)
```

But, this can get repetitive if you are computing a bunch of statistics. Easier (but perhaps more dangerous!) is to change the overall option,
