...
We can assume so, because all the observed counts are so much bigger than 5. 

Now, we can use the `chisq_test()` function to do the test!

```{r}
GSS %>%
  chisq_test(response = general_happiness, p = c(0.14, 0.56, 0.3))
```

Our test statistic is 0.218, we have 2 degrees of freedom, and our p-value is 0.897. 

What is the conclusion to the test?

We fail to reject the null, we have evidence to suggest the null hypothesized proportions are correct. 
...
Again, because our observed counts are all significantly greater than 5, we can assume the expected counts would all be greater than 5. 
...
Our test statistic is 202. Our degrees of freedom are 8 and our p-value is approximately 0. 

What is the conclusion of the test?

We reject the null hypothesis. We have evidence to suggest there is a relationship between marital status and general happiness. 
