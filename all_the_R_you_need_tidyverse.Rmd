...
GSS %>%
  chisq_test(
    ...
    explanatory = NULL, p = c(0.1, 0.9)
  )

GSS %>%
  specify(response = self_emp_or_works_for_somebody, success = "Self-employed") %>%
  calculate(stat = "prop") %>%
...
GSS %>%
  chisq_test(
    response = should_marijuana_be_made_legal,
...
GSS %>%
  drop_na(born_in_us) %>%
  t_test(response = highest_year_of_school_completed, mu = 12, alternative = "greater")
...
summary(a1)
TukeyHSD(a1, conf.level = 0.90) # differences
