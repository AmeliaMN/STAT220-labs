...
```{r}
gf_histogram(~number_of_hours_worked_last_week, data = GSS, binwidth = 5, 
             title = "Number of hours worked last week", xlab = "Hours")
```
...
We do get a warning, because there are 967 rows with missing values. We need to consider whether there are issues of non-response bias that may affect our results. 

Then, we consider who a person works for-- themselves (self-employed) or someone else. 

```{r}
gf_bar(~self_emp_or_works_for_somebody, data = GSS, 
       title = "Employer status", xlab = "")
```
...
```{r}
gf_boxplot(number_of_hours_worked_last_week ~ self_emp_or_works_for_somebody, data = GSS, 
           title = "Hours worked by employer status", xlab = "", ylab = "Hours")
```
...
```{r}
t.test(number_of_hours_worked_last_week ~ self_emp_or_works_for_somebody, data = GSS)
```
